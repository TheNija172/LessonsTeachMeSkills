...
public class User {
    ...
    static public boolean validation(String login, String password, String confirmPassword) throws PasswordValidationException, LoginValidationException {
        if(login.length() >= 20 || login.contains(" "))
            throw new LoginValidationException();
        if(password.length() >= 20 || !password.matches(".*\\d.*") || !password.equals(confirmPassword))
